...
%------------------------------------------------------------------
    ...
    methods
        ...
        %------------------------------------------------------------------
            ...
            trackdisplacements = vecnorm( conv2(obj.track_c,[1,-1],'same') );  %先用[1,-1]对track_c卷积(后一列减去前一列)，再取每一列的范数（也就是长度）
...
%             obj.dist = cumsum( abs(conv(distFromCenter,[1+1e-10,-1],'same')) );
            ...
            dx = diff(obj.track_c(1,:)); % x 方向上的差值
            dy = diff(obj.track_c(2,:)); % y 方向上的差值
            d2x = diff(dx); % x 方向上的二阶导数
            d2y = diff(dy); % y 方向上的二阶导数
            curvature = (d2y.*dx(2:end) - d2x.*dy(2:end))./((dx(2:end).^2 + dy(2:end).^2).^(3/2)); % 曲率
            curvature = padarray(curvature, [0 1], 'replicate', 'pre'); % 添加第一个值
            curvature = padarray(curvature, [0 1], 'replicate', 'post'); % 添加最后一个值
            obj.curvature = curvature;
        end
        
        
        function [pos_c, psi_c, R_c, dist] = getTrackInfo(obj,dist)
        %------------------------------------------------------------------
        ...
        %------------------------------------------------------------------
            ...
            for idx = 1:size(trackdata,1)
                if strcmp( trackdata{idx,1},'s')
                    ...
                    % calculate new track
                    newtrack_l = r_IV + w/2*A_IV(:,2) + (0:ds:dist).*A_IV(:,1);  %左侧道路[XY]序列点
                    newtrack_r = r_IV - w/2*A_IV(:,2) + (0:ds:dist).*A_IV(:,1);
                    newpsi_c   = th0 * ones(1,size(newtrack_l,2));                      %道路航向角序列点
                    % update current position
                    r_IV = r_IV + dist*A_IV(:,1);                                                      %这一段的终点
